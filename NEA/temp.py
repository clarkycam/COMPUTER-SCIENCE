...
for row, (move, move_prime) in enumerate(moves):
    btn = customtkinter.CTkButton(moves_frame,text=move,width=btn_width,height=btn_height,font=move_font,command=lambda m=move: apply_move(m))
    btn.grid(row=row, column=0, padx=4, pady=3)

    btn_p = customtkinter.CTkButton(moves_frame,text=move_prime.replace("prime", "'"),width=btn_width,height=btn_height,font=move_font,command=lambda m=move_prime: apply_move(m))
